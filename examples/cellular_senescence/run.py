#!/usr/bin/env python3
"""Autonomous investigation of cellular senescence transcriptomics using real GEO data.

Uses GSE130727 (human fibroblast replicative senescence) to investigate
molecular mechanisms driving the senescent phenotype via agentic discovery.
"""
...
GEO_ACCESSION = "GSE19018"
GROUP_COLUMN = "growth stage"
CONTROL_LABEL = "Young"
TREATMENT_LABEL = "Old"

ALTERNATIVE_ACCESSIONS = ["GSE41714"]


def _load_expression_data() -> tuple:
    """Load GEO senescence data with fallback to alternative accessions."""
    ...
    for accession in accessions:
        try:
            ...
            expr, groups, meta = load_geo_dataset(
                accession,
                group_column=GROUP_COLUMN if accession == GEO_ACCESSION else None,
                control_label=CONTROL_LABEL if accession == GEO_ACCESSION else None,
                treatment_label=TREATMENT_LABEL if accession == GEO_ACCESSION else None,
            )
    ...
    raise RuntimeError(
        f"Could not load any senescence dataset. Last error: {last_error}\n"
...
def main() -> None:
    cfg = get_config()
    print("=" * 60)
    print("Agentic Scientific Discovery — Cellular Senescence")
    print("=" * 60)
    ...
    orch_cfg = OrchestratorConfig(
        research_question=(
            "What biological pathways and molecular mechanisms drive "
        ...
        tissue="fibroblast",
        condition="replicative senescence",
    )
