#!/usr/bin/env python3
"""Autonomous investigation of neuroinflammation in Alzheimer's disease using real GEO data.

Uses GSE5281 (brain transcriptomics, AD vs control) to investigate
neuroinflammatory pathways and immune mechanisms dysregulated in AD.
"""
...
ALTERNATIVE_ACCESSIONS = ["GSE44770", "GSE33000"]


def _load_expression_data() -> tuple:
    """Load GEO Alzheimer's brain transcriptomics data with fallback accessions."""
    ...
    raise RuntimeError(
        f"Could not load any Alzheimer's dataset. Last error: {last_error}\n"
...
def main() -> None:
    cfg = get_config()
    print("=" * 60)
    print("Agentic Scientific Discovery — Alzheimer's Neuroinflammation")
    print("=" * 60)
    ...
    orch_cfg = OrchestratorConfig(
        research_question=(
            "What neuroinflammatory pathways and immune mechanisms are "
            "dysregulated in Alzheimer's disease brain tissue?"
        ),
        available_data_keys=[
            "expression_matrix",
            "sample_groups",
